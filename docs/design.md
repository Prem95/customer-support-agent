...
The design that I have chosen is a simple stateful workflow that first analyses the intent from the message and decides which node to move on to. 

This is done via the conditional edge that routes the decisions to either the retrieval, recommendation, failure node and the node itself if no definitive routing can be made. 

Each node is checkpointed in the memory and can be accessed by the thread_id
...
| `deploy.yml` | push to `main`                        | Build and push both images tagged with the Git SHA, `terraform apply` with that tag, wait for `ecs services-stable`, output the frontend URL |
...
Run the `application_url_command` output to get the console address to view the Support Agent. `terraform destroy` removes everything.
